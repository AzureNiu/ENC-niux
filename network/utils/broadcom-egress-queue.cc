...
namespace ns3 {
	...
	NS_OBJECT_TEMPLATE_CLASS_DEFINE(BEgressQueue, Packet);
	NS_OBJECT_TEMPLATE_CLASS_DEFINE(BEgressQueue, QueueDiscItem);
	...
	TypeId BEgressQueue<Item>::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::BEgressQueue")
			...
			.AddAttribute("MaxBytes",
				...
				MakeDoubleAccessor(&BEgressQueue<Item>::m_maxBytes),
				MakeDoubleChecker<double>())
			.AddTraceSource ("BeqEnqueue", "Enqueue a packet in the BEgressQueue. Multiple queue",
					MakeTraceSourceAccessor (&BEgressQueue<Item>::m_traceBeqEnqueue))
			.AddTraceSource ("BeqDequeue", "Dequeue a packet in the BEgressQueue. Multiple queue",
					MakeTraceSourceAccessor (&BEgressQueue<Item>::m_traceBeqDequeue))
			;
	...
	template <typename Item>
	bool
		BEgressQueue<Item>::DoEnqueue(Ptr<Packet> p, uint32_t qIndex)
	{
	...
	template <typename Item>
	bool
		BEgressQueue<Item>::Enqueue(Ptr<Packet> p, uint32_t qIndex)
	{
		NS_LOG_FUNCTION(this << p);
		//
		// If DoEnqueue fails, Queue<Item>::Drop is called by the subclass
		//
	...
	template <typename Item>
	bool
		BEgressQueue<Item>::DoEnqueue(Ptr<Packet> p)	//for compatiability
	{
	...
	{
		NS_ASSERT_MSG(false, "BEgressQueue<Item>::DoDequeue not implemented");
		return 0;
	}
